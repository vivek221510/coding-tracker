...
function SquadDetails() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await api.get(`/squads/${squadId}/requests`);
  ...
  const promoteMember = async memberId => {
    try {
      ...
      await api.patch(`/squads/${squadId}/promote-admin/${memberId}`);
  ...
  const demoteMember = async memberId => {
    try {
      ...
      await api.patch(`/squads/${squadId}/demote-admin/${memberId}`);
    } catch (error) {
      console.log(error.response?.data);
    }
  };
  ...
  const removeMember = async memberId => {
    try {
      ...
      await api.delete(`/squads/${squadId}/members/${memberId}`);
    } catch (error) {
      console.log(error.response?.data);
    }
  };
  ...
  const acceptRequest = async requestId => {
    try {
      await api.patch(`/squads/requests/${requestId}/accept`);
  ...
  const rejectRequest = async requestId => {
    try {
      await api.patch(`/squads/requests/${requestId}/reject`);
      fetchRequests();
    } catch (error) {
      console.log(error.response?.data);
    }
        ...
        <div className="p-8 grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <h2 className="text-5xl font-bold mt-3">{squad.admins.length}</h2>
            </div>
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Squad Members</h2>
              ...
              >
                {squad.members.map(member => {
                  const isCreator = squad.createdBy._id === member.user._id;
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="flex gap-2 mt-3">
                            ...
                            {isAdmin && !isCreator && (
                              <button
                                onClick={() => demoteMember(member.user._id)}
                                className="
                                      bg-orange-600
                                      hover:bg-orange-700
                                      px-3
